...
def get_distance_matrix(raw, n_pred=None):
    ...
    covariance_matrix = pd.DataFrame(
        np.abs(np.corrcoef(raw.T.loc[potential_pred])), index=potential_pred, columns=potential_pred
    ).fillna(0)
...
class MultiNet:
    ...
    def build(self, inputdims):
        ...
        for layer in self.NN_parameters["architecture"]:
            if layer["type"].lower() == "dense":
                outputs = [Dense(layer["neurons"], activation=layer["activation"])(output) for output in outputs]
            elif layer["type"].lower() == "dropout":
                outputs = [Dropout(layer["rate"], seed=self.seed)(output) for output in outputs]
            else:
                print("Unknown layer type.")

        outputs = [Dense(self.sub_outputdim, activation="softplus")(output) for output in outputs]
        ...
        model.compile(optimizer=keras.optimizers.Adam(learning_rate=self.NN_parameters["learning_rate"]), loss=loss)
    ...
    def fit(
        ...
        result = model.fit(
            ...
            callbacks=[EarlyStopping(monitor="val_loss", patience=self.NN_parameters["patience"])],
    ...
    def predict(self, raw, imputed_only=False, policy="restore"):

        norm_raw = np.log1p(raw)

        inputs = [norm_raw.loc[:, predictors].values.astype(np.float32) for predictors in self.predictors]
        ...
        imputed = pd.concat([predicted, not_predicted], axis=1).loc[raw.index, raw.columns].values
    ...
    def filter_genes(self, gene_metric, threshold, NN_lim=None):  # assumes gene_metric is sorted
    ...
    def setTargets(self, data, mode="random"):
        ...
        if mode == "progressive":
            ...
            self.targets = np.random.choice(data.columns, [n_subsets, self.sub_outputdim], replace=False)
    ...
    def setPredictors(self, covariance_matrix, ntop=5):
        ...
        for i, targets in enumerate(self.targets):
            ...
            subMatrix = covariance_matrix.loc[targets, genes_not_in_target]
            ...
            print("Net {}: {} predictors, {} targets".format(i, len(np.unique(predictors)), len(targets)))
